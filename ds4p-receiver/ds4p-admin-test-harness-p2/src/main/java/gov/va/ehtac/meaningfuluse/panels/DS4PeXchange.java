...
public class DS4PeXchange extends Panel {
    ...
    private Button unEncryptBTN = new Button("View Decrypted XML - Including Masking");
    private Button unEncryptDocOnlyBTN = new Button("View Decrypted XML - Document Only");
    ...
    public DS4PeXchange() {
            ...
            public void valueChange(Property.ValueChangeEvent event) {
                ...
                if (rowId != null) {
                    ...
                    if (docType.equals("Consent Notes")) {
                        ...
                        unEncryptDocOnlyBTN.setEnabled(false);
                        ...
                        viewXMLBTN.setEnabled(true);
                        unEncryptDocOnlyBTN.setEnabled(true);
                        unEncryptBTN.setEnabled(true);
                        transformBTN.setEnabled(true);
                        viewConsentPDFBTN.setEnabled(false);
                        viewConsentXACMLBTN.setEnabled(false);
        ...
        pArea.setWidth("100%");
        pArea.setHeight("250px");
        pArea.setStyleName(Runo.PANEL_LIGHT);
        pArea.setCaption("No Documents Selected");
        area.setContentMode(Label.CONTENT_XHTML);
        pArea.addComponent(area);
        v.addComponent(pArea);
        
        HorizontalLayout h2 = new HorizontalLayout();
        h2.setWidth("100%");
        viewXMLBTN.setStyleName(Runo.BUTTON_SMALL);
        unEncryptDocOnlyBTN.setStyleName(Runo.BUTTON_SMALL);
        ...
        viewXMLBTN.setEnabled(false);
        unEncryptDocOnlyBTN.setEnabled(false);
        unEncryptBTN.setEnabled(false);
        transformBTN.setEnabled(false);
        viewConsentPDFBTN.setEnabled(false);
        viewConsentXACMLBTN.setEnabled(false);
        
        h2.addComponent(viewXMLBTN);
        h2.addComponent(unEncryptDocOnlyBTN);
        ...
        viewXMLBTN.addListener(new Button.ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                ...
                if (rowId != null) {
                    getDocument();
                    pArea.setCaption("Selected Document (Raw)");
                    area.setValue(convertXMLtoXHTML(currentDocument));
                    pArea.requestRepaint();
                }
                else {
        ...
        unEncryptDocOnlyBTN.addListener(new Button.ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                if (currentDocument.length() == 0) {
                    ...
                    try {
                        ...
                        pArea.setCaption("Selected Document Decrypted - Document Only");
                        area.setValue(convertXMLtoXHTML(decryptDocument));
                        pArea.requestRepaint();
                    }
                    catch (Exception ex) {
                        ex.printStackTrace();
            ...
            public void buttonClick(ClickEvent event) {
                if (currentDocument.length() == 0) {
                    ...
                    try {
                        ...
                        pArea.setCaption("Selected Document Decrypted - Document and Masking");
                        area.setValue(convertXMLtoXHTML(decryptDocument));
                        pArea.requestRepaint();
                    }
                    catch (Exception ex) {
